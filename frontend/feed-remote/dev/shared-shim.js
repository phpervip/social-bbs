 ...
 * '@b/shared' → this file so the exposed pages can be verified
  ...
  emit(event, payload) {
    (bus._listeners[event] || []).forEach((fn) => {
      try {
        ...
        console.error('[feed-standalone bus]', e);
      }
    });
  },
  _listeners: {},
...
const api = {
  devLogin: async (userId) => {
    const data = await http.post('/dev/login', { user_id: userId });
    localStorage.setItem('b_token', data.token);
    bus.emit(bus.events.AUTH_LOGIN, data.token);
    return data;
  },
  devUsers: () => http.get('/dev/users'),
  createPost: (body) => http.post('/feed/post', body),
  getHomeTimeline: (params) => http.get('/feed/home', { params }),
  getPost: (id) => http.get(`/feed/post/${id}`),
  deletePost: (id) => http.delete(`/feed/post/${id}`),
  likePost: (postId) => http.post('/feed/like', { post_id: postId }),
  unlikePost: (postId) => http.delete('/feed/like', { data: { post_id: postId } }),
  addComment: (body) => http.post('/feed/comment', body),
  getComments: ({ post_id, cursor, limit }) =>
    http.get(`/feed/post/${post_id}/comments`, { params: { cursor, limit } }),
...
if (typeof document !== 'undefined') {
  ...
  style.textContent = `@keyframes feedShimmer { 0%,100% { opacity: 1 } 50% { opacity: .45 } }`;
